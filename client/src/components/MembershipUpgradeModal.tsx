...
export default function MembershipUpgradeModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
      <div
        className={`bg-white rounded-2xl w-full max-w-md overflow-hidden transform transition-all duration-500 ${showContent ? "scale-100 opacity-100" : "scale-90 opacity-0"}`}
      >
        {/* 顶部装饰 */}
        <div
        ...
        >
          ...
          {/* 装饰元素 */}
          <Sparkles
            className="absolute top-4 left-4 text-white/60 animate-pulse"
            size={20}
          />
          <Sparkles
            className="absolute top-6 right-6 text-white/60 animate-pulse delay-75"
            size={16}
          />
          <Sparkles
            className="absolute bottom-4 left-8 text-white/60 animate-pulse delay-150"
            size={18}
          />
          <Sparkles
            className="absolute bottom-6 right-8 text-white/60 animate-pulse delay-100"
            size={14}
          />
        </div>
