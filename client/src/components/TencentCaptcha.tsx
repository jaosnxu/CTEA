...
interface TencentCaptchaSDK {
  new (
    appId: string,
    callback: (res: { ret: number; ticket: string; randstr: string; errorCode?: number; errorMessage?: string }) => void,
    options?: {
      lang?: string;
      type?: string;
...
export function TencentCaptcha({ 
  ...
  // 获取本地化错误消息
  const getLocalizedError = useCallback((key: string) => {
    const messages = CAPTCHA_ERROR_MESSAGES[key];
    if (messages) {
      return messages[language] || messages['ru'];
    }
    return CAPTCHA_ERROR_MESSAGES['failed'][language] || CAPTCHA_ERROR_MESSAGES['failed']['ru'];
  }, [language]);
    ...
    captchaRef.current = new window.TencentCaptcha(
      ...
      (res) => {
        ...
        if (res.ret === 0) {
          ...
          // 验证失败
          const errorKey = res.errorCode === 6 ? 'expired' : 
                          res.errorCode === 21 ? 'frequent' : 'failed';
...
export function getCaptchaErrorMessage(errorKey: string, lang: string): string {
  ...
  return CAPTCHA_ERROR_MESSAGES['failed'][lang] || CAPTCHA_ERROR_MESSAGES['failed']['ru'];
}
