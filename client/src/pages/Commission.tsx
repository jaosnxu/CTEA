...
import { ChevronLeft, TrendingUp, DollarSign, Users, Calendar, ArrowUpRight } from "lucide-react";
          ...
          <div className="bg-gradient-to-br from-purple-600 to-indigo-600 rounded-2xl p-6 text-white shadow-lg">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <div className="text-4xl font-bold">₽{COMMISSION_DATA.totalEarnings}</div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div>
                ...
                <div className="text-lg font-bold">₽{COMMISSION_DATA.availableBalance}</div>
              </div>
              <div>
                <div className="text-xs text-white/70 mb-1">待结算</div>
                <div className="text-lg font-bold">₽{COMMISSION_DATA.pendingBalance}</div>
              </div>
              <div>
                <div className="text-xs text-white/70 mb-1">已Вывод</div>
                <div className="text-lg font-bold">₽{COMMISSION_DATA.withdrawnTotal}</div>
              </div>
            </div>
        ...
        <div className="px-4 pb-4">
          <div className="bg-white rounded-xl p-4 grid grid-cols-3 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600 mb-1">{COMMISSION_DATA.currentLevel}</div>
              <div className="text-xs text-gray-500">当前等级</div>
            </div>
            <div className="text-center border-l border-r border-gray-100">
              <div className="text-2xl font-bold text-purple-600 mb-1">{COMMISSION_DATA.commissionRate}%</div>
              <div className="text-xs text-gray-500">佣金比例</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600 mb-1">{COMMISSION_DATA.totalReferrals}</div>
        ...
        <div className="px-4 pb-4">
          <div className="bg-white rounded-xl p-1 flex gap-1">
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                filter === "order" ? "bg-purple-600 text-white" : "text-gray-600"
              }`}
            >
              Комиссия за заказ
            </button>
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                filter === "referral" ? "bg-purple-600 text-white" : "text-gray-600"
              }`}
            >
              Реферальный бонус
            </button>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      ...
                      {record.type === "order" ? "Комиссия за заказ" : "Реферальный бонус"}
                    </span>
                  </div>
                  
                  {record.product && (
                    <div className="text-sm text-gray-600 mb-1">{record.product}</div>
                  )}
                  {record.note && (
                    <div className="text-sm text-gray-600 mb-1">{record.note}</div>
                  )}
                  {record.orderAmount && (
                    <div className="text-xs text-gray-500">Сумма заказа: ₽{record.orderAmount}</div>
                  )}
                </div>
                
                <div className="text-right">
                  <div className="text-xl font-bold text-purple-600">+₽{record.amount}</div>
                  <div className={`text-xs mt-1 ${
                    record.status === "settled" ? "text-green-600" : "text-orange-600"
                  }`}>
        ...
        <div className="px-4 pb-4">
          <div className="bg-white rounded-xl p-4">
            ...
            <div className="space-y-2 text-sm text-gray-600">
              <p>• Комиссия за заказ：推广订单成交后获得 {COMMISSION_DATA.commissionRate}% 佣金</p>
              <p>• Реферальный бонус：邀请新用户注册并Готово首单，获得 ₽50 奖励</p>
