...
import { ChevronLeft, CreditCard, Building2, Wallet, AlertCircle } from "lucide-react";
...
export default function CommissionWithdraw() {
  ...
  const fee = selectedMethodData ? (withdrawAmount * selectedMethodData.fee / 100) : 0;
  ...
  const handleWithdraw = () => {
    ...
    if (withdrawAmount < (selectedMethodData?.minAmount || 100)) {
      toast.error(`Минимальная сумма вывода ₽${formatCurrency(selectedMethodData?.minAmount)}`);
      return;
    }
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-4 pb-4">
          <div className="bg-white rounded-xl p-4">
            <label className="block text-sm font-semibold mb-3">Вывод金额</label>
            ...
            <div className="grid grid-cols-4 gap-2 mt-3">
              {QUICK_AMOUNTS.map((quickAmount) => (
                ...
                >
                  {quickAmount === availableBalance ? "Все" : `₽${formatCurrency(quickAmount)}`}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="px-4 pb-4">
          <div className="bg-white rounded-xl p-4">
            <label className="block text-sm font-semibold mb-3">Вывод方式</label>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right">
                      {method.fee > 0 ? (
                        <div className="text-xs text-gray-500">手续费 {method.fee}%</div>
                      ) : (
                        <div className="text-xs text-green-600">免手续费</div>
                      )}
                      <div className="text-xs text-gray-400 mt-1">最低₽{method.minAmount}</div>
                    </div>
                  </button>
                );
              })}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4">
          <Button
            ...
            disabled={!amount || withdrawAmount <= 0 || withdrawAmount > availableBalance}
