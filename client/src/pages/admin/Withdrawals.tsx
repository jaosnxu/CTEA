...
const translations = {
  ru: {
    ...
    modal: {
      ...
      approveMessage: 'Вы уверены, что хотите одобрить эту заявку на вывод средств?',
...
const VoucherModal: React.FC<VoucherModalProps> = ({ isOpen, voucher, onClose, lang }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-gray-900">📄 {t.voucher.title}</h3>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-mono font-bold text-green-700">{voucher.voucherNo}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-2xl text-green-600">₽ {voucher.amount?.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">{t.voucher.created}:</span>
              <span className="text-sm">{new Date(voucher.createdAt).toLocaleString()}</span>
            </div>
          </div>
        </div>
...
export default function WithdrawalsPage() {
  ...
  const [voucherModal, setVoucherModal] = useState<{ isOpen: boolean; voucher: any }>({ isOpen: false, voucher: null });
  ...
  const handleApprove = async () => {
    ...
    setProcessing(true);
    try {
      const response = await fetch(`/api/finance/withdrawals/${confirmModal.withdrawalId}/approve`, {
        method: 'POST',
      ...
      if (data.success) {
        setSuccessMessage(confirmModal.type === 'approve' ? t.success.approved : t.success.rejected);
    ...
    <AdminLayout>
      <div className="p-6">
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-yellow-50 rounded-xl p-4 border border-yellow-200">
              <div className="text-yellow-600 text-sm font-medium">{t.stats.pending}</div>
              <div className="text-2xl font-bold text-yellow-700">{stats.pending.count}</div>
              <div className="text-sm text-yellow-600">{formatAmount(stats.pending.total)}</div>
            </div>
            <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
              <div className="text-blue-600 text-sm font-medium">{t.stats.processing}</div>
              <div className="text-2xl font-bold text-blue-700">{stats.processing.count}</div>
              <div className="text-sm text-blue-600">{formatAmount(stats.processing.total)}</div>
            </div>
            <div className="bg-green-50 rounded-xl p-4 border border-green-200">
              <div className="text-green-600 text-sm font-medium">{t.stats.completed}</div>
              <div className="text-2xl font-bold text-green-700">{stats.completed.count}</div>
              <div className="text-sm text-green-600">{formatAmount(stats.completed.total)}</div>
            </div>
            <div className="bg-red-50 rounded-xl p-4 border border-red-200">
              <div className="text-red-600 text-sm font-medium">{t.stats.rejected}</div>
              <div className="text-2xl font-bold text-red-700">{stats.rejected.count}</div>
              <div className="text-sm text-red-600">{formatAmount(stats.rejected.total)}</div>
            </div>
          </div>
        )}
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.table.influencer}</th>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.table.createdAt}</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {withdrawals.map((w) => (
                    <tr key={w.id} className="hover:bg-gray-50">
                      <td className="px-4 py-4 text-sm font-mono text-gray-900">#{w.id}</td>
                      <td className="px-4 py-4">
                        <div className="text-sm font-medium text-gray-900">
                          {w.influencer?.nickname || w.influencer?.realName?.ru || `ID: ${w.influencerId}`}
                        </div>
                        {w.influencer?.user?.phone && (
                          <div className="text-xs text-gray-500">{w.influencer.user.phone}</div>
                        )}
                      </td>
                      <td className="px-4 py-4 text-sm font-bold text-gray-900">{formatAmount(w.amount)}</td>
                      <td className="px-4 py-4">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusStyle(w.status)}`}>
                          {t.status[w.status]}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-sm text-gray-500">{formatDate(w.createdAt)}</td>
                      <td className="px-4 py-4">
                        <div className="flex gap-2">
                          {w.status === 'PENDING' && (
                            <>
                              <button
                                onClick={() => setConfirmModal({ isOpen: true, type: 'approve', withdrawalId: w.id })}
                                className="px-3 py-1 bg-green-500 text-white text-xs rounded-lg hover:bg-green-600 transition-colors"
                              >
                                ✓ {t.actions.approve}
                              </button>
                              <button
                                onClick={() => setConfirmModal({ isOpen: true, type: 'reject', withdrawalId: w.id })}
        ...
        <ConfirmModal
          ...
          title={confirmModal.type === 'approve' ? t.modal.approveTitle : t.modal.rejectTitle}
          message={confirmModal.type === 'approve' ? t.modal.approveMessage : t.modal.rejectMessage}
