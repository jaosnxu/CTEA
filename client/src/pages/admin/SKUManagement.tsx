...
export default function SKUManagementPage() {
  ...
  const [activeTab, setActiveTab] = useState<"skus" | "stores" | "alerts">(
    "skus"
  );
  ...
  const updateSKU = async (id: number, field: string, value: boolean) => {
    try {
      ...
      if (data.success) {
        setSKUs(
          skus.map(sku => (sku.id === id ? { ...sku, [field]: value } : sku))
        );
                  ...
                  skus.map(sku => (
                    <tr key={sku.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <button
                          onClick={() =>
                            updateSKU(sku.id, "isActive", !sku.isActive)
                          }
                  ...
                  alerts.map(alert => (
                    <tr key={alert.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">
                        {typeof alert.storeName === "object"
                          ? alert.storeName[lang]
                          : alert.storeName}
                      </td>
