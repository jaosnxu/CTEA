...
import { ArrowLeft, MapPin, ChevronRight, Store, Clock, Wallet, Truck, Tag } from "lucide-react";
...
export default function Checkout() {
  ...
  const { t } = useLanguage();
  const { 
    drinkCart, drinkCartTotal, clearDrinkCart, updateDrinkCartQuantity, removeFromDrinkCart,
    mallCart, mallCartTotal, clearMallCart, updateMallCartQuantity, removeFromMallCart,
    userPoints, deductPoints, addOrder,
    coupons, selectedCouponId, selectCoupon,
  ...
  const availableGiftCards = giftCards.filter(c => c.status === "active" && c.balance > 0);
  ...
  const couponDiscount = selectedCoupon && cartTotal >= selectedCoupon.minAmount ? selectedCoupon.discount : 0;
  ...
  const giftCardPayment = useGiftCardPayment && selectedGiftCard ? Math.min(giftCardBalance, subtotal) : 0;
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      <div className="bg-white px-4 py-3 flex items-center gap-3 sticky top-0 z-10 shadow-sm">
        ...
        <h1 className="font-bold text-lg">{t("pages_checkout_提交订单")} ({isDrink ? t("pages_checkout_餐饮") : t("pages_checkout_商城")})</h1>
      </div>
        ...
        <div className="bg-white mt-3 px-4 py-4 cursor-pointer">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold">{t("pages_checkout_莫斯科go店")}</span>
            </div>
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Clock size={14} />
            <span>{t("pages_checkout_预计")} {deliveryMethod === "pickup" ? "15-20" : "30-40"} {t("pages_checkout_分钟")}</span>
          </div>
        </div>
      </Link>
          ...
          {cart.map((item) => (
            <div key={item.id} className="flex gap-3">
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between">
                <div>
                  <h3 className="font-bold text-xs mb-1 truncate">{item.name}</h3>
                  {item.specs && (
                    <p className="text-[11px] text-muted-foreground mb-1">{item.specs}</p>
                  )}
                  {item.toppings && item.toppings.length > 0 && (
                    <div className="border-l-2 border-gray-200 pl-2 mt-1 space-y-0.5">
                      {item.toppings.map((topping, index) => (
                        <div key={index} className="flex items-center gap-1 text-[10px] text-gray-600">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 rounded-full px-1 py-1">
                    ...
                    <span className="text-sm font-bold min-w-[24px] text-center px-2">{item.quantity}</span>
                    <button
                      onClick={() => handleUpdateQuantity(item.id, item.quantity + 1)}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {selectedCoupon ? (
              <div className="text-xs text-primary">-{formatCurrency(couponDiscount)}</div>
            ) : (
              <div className="text-xs text-muted-foreground">
                {coupons.filter(c => c.available && cartTotal >= c.minAmount).length} {t("pages_checkout_张可用")}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white mt-3 px-4 py-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Tag size={20} className="text-primary" />
              <div>
                <div className="font-bold">{t("pages_checkout_使用礼品卡")}</div>
                <div className="text-xs text-muted-foreground">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {t("pages_checkout_余额")}: {formatCurrency(card.balance.toFixed(2))}
                      </div>
                    </div>
                    {selectedGiftCardId === card.id && (
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {t("pages_checkout_可用")} {userPoints} {t("pages_checkout_积分")} (≈₽{pointsValue.toFixed(2)})
                </div>
              </div>
            </div>
            <Switch
      ...
      <div className="bg-white mt-3 px-4 py-4">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-muted-foreground">{t("pages_checkout_商品小计")}</span>
            <span>{formatCurrency(cartTotal.toFixed(2))}</span>
          </div>
          {deliveryMethod === "delivery" && (
            <div className="flex justify-between">
              <span className="text-muted-foreground">{t("pages_checkout_配送费")}</span>
          ...
          {pointsDeduction > 0 && (
            <div className="flex justify-between text-green-600">
              <span>{t("pages_checkout_积分抵扣")} ({Math.ceil(pointsDeduction)} {t("pages_checkout_积分")})</span>
              <span>-{formatCurrency(pointsDeduction.toFixed(2))}</span>
            </div>
          )}
          <div className="flex justify-between pt-2 border-t border-gray-200">
            <span className="font-bold">{t("pages_checkout_合计")}</span>
            <span className="text-primary font-bold text-lg">{formatCurrency(finalTotal.toFixed(2))}</span>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-4 flex items-center gap-3 shadow-lg">
        <div className="flex-1">
          <div className="text-xs text-muted-foreground">{t("pages_checkout_合计")}</div>
          <div className="text-primary font-bold text-xl">{formatCurrency(finalTotal.toFixed(2))}</div>
        </div>
        <Button
