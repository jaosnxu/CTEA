...
async function testLogging() {
  ...
  authLogger.error('Authentication failed', new Error('Invalid token'), { userId: 'test-123' });
  ...
  requestLogger.debug('Request details', { method: 'GET', params: { id: '456' } });
  ...
  queryLogger.info('Executing query', { sql: 'SELECT * FROM users WHERE id = ?', params: ['123'] });
  ...
  console.log('Note: File logs are only created in production mode or for error level.\n');
}
