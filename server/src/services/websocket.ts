...
export class WebSocketService {
  ...
  private setupEventHandlers() {
    this.io.on("connection", (socket: Socket) => {
      ...
      socket.on("unsubscribe:user", (userId: string) => {
        ...
        console.log(
          `[WebSocket] 客户端 ${socket.id} 取消订阅用户订单: ${userId}`
        );
      });
      ...
      socket.on("disconnect", reason => {
        console.log(
          `[WebSocket] 客户端断开连接: ${socket.id}, 原因: ${reason}`
        );
  ...
  public emitOrderStatusChange(event: OrderStatusChangeEvent) {
    ...
    console.log(
      `[WebSocket] 推送订单状态变更: ${event.orderId}, 状态: ${status}, 提货码: ${pickupCode}`
    );
...
export function getWebSocketService(): WebSocketService {
  if (!wsService) {
    throw new Error(
      "[WebSocket] 服务未初始化，请先调用 initWebSocketService()"
    );
  }
  return wsService;
}
