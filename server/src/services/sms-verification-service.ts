...
import { validatePhoneNumber, getLocalizedError } from './sms/sms-provider.interface';

// ==================== 类型定义 ====================

/** 验证码用途 */
export type VerificationPurpose = 'LOGIN' | 'REGISTER' | 'RESET_PASSWORD' | 'SENSITIVE_ACTION';
...
export class SmsVerificationService {
  ...
  async sendCode(request: SendCodeRequest): Promise<SendCodeResponse> {
    const { phone, purpose, ticket, randstr, userIp, language = 'ru' } = request;
    ...
    // ==================== 第六步：存储到数据库 ====================
    console.log('\n[Step 5] 存储到数据库...');
    const stored = await this.storeCode(phone, code, purpose, expiresAt, userIp);
    if (!stored) {
      console.log('❌ 存储失败');
      return {
        success: false,
    ...
    if (!smsResult.success) {
      ...
      return {
        ...
        errorMessage: smsResult.errorMessage || getLocalizedError('send_failed', language),
      };
    }
   ...
   * 检查发送冷却时间
   */
  private async checkCooldown(phone: string): Promise<{ isLimited: boolean; remaining: number }> {
    const db = await getDb();
    if (!db) {
   ...
   * 使旧验证码失效
   */
  private async invalidateOldCodes(phone: string, purpose: VerificationPurpose): Promise<void> {
    const db = await getDb();
    if (!db) return;
   ...
   * 增加尝试次数
   */
  private async incrementAttemptCount(id: number, newCount: number): Promise<void> {
    const db = await getDb();
    if (!db) return;
  ...
  private getVerifyErrorMessage(key: string, lang: string): string {
    ...
    return messages[key]?.[lang] || messages[key]?.['ru'] || 'Ошибка проверки кода.';
  }
}
