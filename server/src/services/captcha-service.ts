...
export class CaptchaService {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        return {
          ...
          captchaAppSecret: process.env.TENCENT_CAPTCHA_APP_SECRET || 'TEST_APP_SECRET',
          captchaType: 'SLIDE',
          isActive: true
        };
      }
   ...
   */
  async getVerificationRule(scenario: VerificationScenario): Promise<VerificationRule | null> {
    const db = await getDb();
    if (!db) {
      console.error('[CaptchaService] Database not available');
      return null;
   ...
   * 基于信任分和每日配额
   */
  async canSkipVerification(userId: number, scenario: VerificationScenario): Promise<boolean> {
    // 永不降级的红线场景
   ...
   * @returns 校验结果
   */
  async verifyTicket(request: CaptchaVerifyRequest): Promise<CaptchaVerifyResponse> {
    ...
    try {
      ...
      return {
        ...
        errorMessage: success ? undefined : this.getCaptchaErrorMessage(captchaCode)
      };
  ...
  private async logVerifyResult(params: {
    ...
    try {
      ...
      console.log(`[CaptchaService] Verify log recorded: ${params.verifyResult ? 'SUCCESS' : 'FAILED'}`);
    } catch (error) {
   ...
   * 检查场景是否需要验证码
   */
  async requiresCaptcha(scenario: VerificationScenario, userId?: number): Promise<{
    required: boolean;
    captchaType?: CaptchaType;
    canSkip?: boolean;
  }> {
