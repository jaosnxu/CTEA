...
async function sendViaSmsRu(
  ...
  try {
    ...
    // 支持测试模式
    if (config.test_mode === 'true' || process.env.SMS_RU_TEST_MODE === 'true') {
...
export class SmsService {
  ...
  private getDefaultProviderConfigs(region: string): ProviderConfig[] {
    if (region === 'RU' || region === 'RUSSIA') {
      return [
        { providerName: 'SMS_RU', region: 'RU', priority: 1, apiConfig: {}, isActive: true },
        { providerName: 'MTS', region: 'RU', priority: 2, apiConfig: {}, isActive: true }
      ];
    }
    
    // 国际/中国
    return [
      { providerName: 'TWILIO', region: 'INTERNATIONAL', priority: 1, apiConfig: {}, isActive: true }
    ];
  }
  ...
  async checkRateLimit(phone: string): Promise<RateLimitStatus> {
    const db = await getDb();
    if (!db) {
      return { isLimited: false, cooldownRemaining: 0, dailyCount: 0, dailyLimit: DAILY_LIMIT_PER_PHONE };
    }
   ...
   * 核心方法：发送验证码
   */
  async sendVerificationCode(request: SmsSendRequest): Promise<SmsSendResponse> {
    ...
    if (rateLimit.isLimited) {
      ...
      console.log(`Daily Count: ${rateLimit.dailyCount}/${rateLimit.dailyLimit}`);
      console.log(`${'='.repeat(60)}\n`);
  ...
  private async sendViaProvider(
    ...
    switch (provider.providerName) {
      ...
      default:
        return { success: false, error: `Unknown provider: ${provider.providerName}` };
    }
  }
  ...
  async verifyCode(request: SmsVerifyRequest): Promise<SmsVerifyResponse> {
    ...
    const db = await getDb();
    if (!db) {
      return { success: false, errorCode: 'DB_ERROR', errorMessage: '服务暂时不可用' };
    }
   ...
   */
  async isCodeVerified(phone: string, purpose: VerificationPurpose, withinMinutes: number = 10): Promise<boolean> {
    const db = await getDb();
    if (!db) return false;
