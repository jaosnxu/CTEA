...
const SENSITIVE_ACTIONS: SensitiveAction[] = [
  ...
  },
  {
    ...
    method: "POST",
    scenario: "STORE_MANAGER_REFUND",
    description: "店长退款",
  },

  // 达人操作
  {
    path: "/api/influencer/withdraw",
    method: "POST",
    scenario: "INFLUENCER_WITHDRAWAL",
    description: "达人提现",
  },
...
export class SecurityMiddleware {
  ...
  async loadSensitiveActionsFromDb(): Promise<SensitiveAction[]> {
    ...
    if (!db) {
      console.warn(
        "[SecurityMiddleware] Database not available, using default config"
      );
    ...
    try {
      ...
      if (rows && rows.length > 0) {
        return rows.map((row: any) => {
          const value =
            typeof row.rule_value === "string"
              ? JSON.parse(row.rule_value)
              : row.rule_value;
          return {
            path: value.path,
            method: value.method,
      ...
      console.error(
        "[SecurityMiddleware] Failed to load sensitive actions:",
        error
      );
      return SENSITIVE_ACTIONS;
    }
  ...
  async isDeviceBlocked(fingerprint: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        "[SecurityMiddleware] Failed to check device block:",
        error
      );
      return false;
    }
  ...
  async logSecurityEvent(log: SecurityAuditLog): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[SecurityMiddleware] Audit log: [${log.severity}] ${log.eventType} - ${log.description}`
      );
    } catch (error) {
      console.error(
        "[SecurityMiddleware] Failed to log security event:",
        error
      );
    }
  }
  ...
  matchSensitiveAction(
    ...
    for (const action of actions) {
      ...
      if (
        normalizedPath.startsWith(actionPath) &&
        normalizedMethod === action.method.toUpperCase()
      ) {
        return action;
      }
    }
  ...
  async performSecurityCheck(
    ...
    // 4. 匹配敏感操作
    const matchedAction = this.matchSensitiveAction(
      path,
      method,
      sensitiveActions
    );
    ...
    // 5. 获取验证规则
    const rule = await captchaService.getVerificationRule(
      matchedAction.scenario
    );

    if (!rule || !rule.isActive) {
      // 规则未配置或已禁用，放行（但记录警告）
      console.warn(
        `[SecurityMiddleware] No active rule for scenario: ${matchedAction.scenario}`
      );
      return { allowed: true };
    }
