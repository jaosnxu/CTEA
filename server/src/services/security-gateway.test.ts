...
describe("SecurityMiddleware - 店长改价拦截测试", () => {
  ...
  it("应该拦截没有验证票据的店长改价请求", async () => {
    ...
    mockExecute.mockImplementation((query: string, params?: any[]) => {
      ...
      if (
        query.includes("security_rules") &&
        query.includes("SENSITIVE_ACTION")
      ) {
        return Promise.resolve([[]]);
      }
...
describe("完整请求拦截日志模拟", () => {
  it("模拟店长改价请求被拦截的完整日志", async () => {
    ...
    console.log(
      '║     "message": "此操作需要完成 点选验证 验证",                     ║'
    );
    console.log(
      '║     "requiredCaptchaType": "CLICK",                                ║'
    );
    console.log(
      '║     "requiresSms": true                                            ║'
    );
