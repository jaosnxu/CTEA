...
export class AuthService {
  ...
  async login(request: LoginRequest): Promise<LoginResponse> {
    ...
    try {
      ...
      return {
        ...
        errorMessage: language === 'ru' ? 'Ошибка входа. Попробуйте позже.' : '登录失败，请稍后重试',
      };
    }
  }
   ...
   * 刷新 JWT Token
   */
  async refreshToken(token: string): Promise<{ token: string; expiresAt: Date } | null> {
   ...
   * 更新用户信息
   */
  async updateUser(userId: number, updates: { nickname?: string; avatar?: string }): Promise<boolean> {
    const db = await getDb();
    if (!db) return false;
   ...
   * 发送新用户注册 Telegram 通知
   */
  private async sendRegistrationNotification(user: User, userIp?: string): Promise<void> {
