 ...
 * 生成俄语通知消息
 */
function generateMessage(type: NotificationType, data: Record<string, any>): string {
...
export class TelegramBotService {
  ...
  private bindTokens: Map<string, { userId: number; expiresAt: Date }> = new Map();
  ...
  generateBindToken(userId: number): string {
    ...
    console.log(`[TelegramBot] 生成绑定 Token: userId=${userId}, token=${token.substring(0, 8)}...`);
   ...
   * 完成绑定（从 Bot 回调）
   */
  async completeBind(token: string, telegramChatId: string): Promise<TelegramBindResponse> {
    ...
    console.log(`[TelegramBot] 绑定成功: userId=${validation.userId}, chatId=${telegramChatId}`);
