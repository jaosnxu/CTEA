  ...
  describe("发送验证码", () => {
    it("应该成功发送验证码", async () => {
      ...
      mockExecute.mockImplementation((sqlObj: unknown) => {
        ...
        // 使旧验证码失效
        if (query.includes("UPDATE") && query.includes("VERIFIED")) {
          return Promise.resolve([{ affectedRows: 0 }]);
        }
        // 存储新验证码
        if (query.includes("INSERT INTO sms_verification_logs")) {
  ...
  describe("校验验证码", () => {
    it("应该成功校验正确的验证码", async () => {
      ...
      mockExecute.mockImplementation((sqlObj: unknown) => {
        ...
        if (
          ...
          query.includes("sms_verification_logs")
        ) {
          return Promise.resolve([
            [
              {
    ...
    it("应该拒绝错误的验证码并增加尝试次数", async () => {
      ...
      mockExecute.mockImplementation((sqlObj: unknown) => {
        ...
        // 增加尝试次数
        if (query.includes("UPDATE") && query.includes("attempts")) {
          return Promise.resolve([{ affectedRows: 1 }]);
        }
        return Promise.resolve([[]]);
      });
...
describe("安全逻辑综合测试", () => {
  it("模拟完整的验证流程日志", async () => {
    ...
    console.log(
      "║ POST /api/sms/send   - 发送验证码（需 Captcha）                    ║"
    );
    console.log(
      "║ POST /api/sms/verify - 校验验证码                                  ║"
    );
    console.log(
      "║ GET  /api/sms/status - 服务状态                                    ║"
    );
    console.log(
      "╚════════════════════════════════════════════════════════════════════╝"
    );
