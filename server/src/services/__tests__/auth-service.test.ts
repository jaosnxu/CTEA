...
describe('AuthService - 用户体系与身份认证测试', () => {
  ...
  describe('无感注册登录', () => {
    it('应该为新用户自动创建账户并登录', async () => {
      ...
      console.log('测试结果:', JSON.stringify({
    ...
    it('应该为已存在用户直接登录', async () => {
      ...
      mockExecute.mockImplementation((query: string) => {
        // 查找用户 - 返回已存在用户
        if (query.includes('SELECT') && query.includes('FROM users') && query.includes('WHERE phone')) {
          return Promise.resolve([[{
            id: 1,
            phone: '+79001234567',
            nickname: 'Иван',
...
describe('M4 用户体系综合测试', () => {
  it('展示完整的用户认证流程', async () => {
    console.log('\n');
    console.log('╔════════════════════════════════════════════════════════════════════╗');
    console.log('║           CHUTEA 智慧中台 - M4 用户体系与身份认证                   ║');
    console.log('╠════════════════════════════════════════════════════════════════════╣');
    console.log('║ 数据库表                                                           ║');
    console.log('║ ─────────────────────────────────────────────────────────────────  ║');
    console.log('║ users: phone(唯一), nickname, avatar, status, last_login_at        ║');
    console.log('╠════════════════════════════════════════════════════════════════════╣');
    console.log('║ API 端点                                                           ║');
    console.log('║ ─────────────────────────────────────────────────────────────────  ║');
    console.log('║ POST /api/auth/login    - 无感注册登录（手机号+验证码）            ║');
    ...
    console.log('╠════════════════════════════════════════════════════════════════════╣');
    console.log('║ 无感注册登录流程                                                   ║');
    console.log('║ ─────────────────────────────────────────────────────────────────  ║');
    console.log('║ 1. 用户输入手机号 + 验证码                                         ║');
    console.log('║ 2. 校验 SMS 验证码                                                 ║');
    console.log('║ 3. 手机号不存在 → 自动创建用户                                     ║');
    console.log('║ 4. 手机号已存在 → 直接登录                                         ║');
    console.log('║ 5. 生成 JWT Token（7天有效期）                                     ║');
    console.log('╠════════════════════════════════════════════════════════════════════╣');
    console.log('║ 安全特性                                                           ║');
    console.log('║ ─────────────────────────────────────────────────────────────────  ║');
    console.log('║ ✅ JWT Token 包含过期时间                                          ║');
    console.log('║ ✅ 后端 Secret 校验                                                ║');
    console.log('║ ✅ 认证中间件保护敏感接口                                          ║');
    console.log('║ ✅ 支持 Token 刷新                                                 ║');
    console.log('╚════════════════════════════════════════════════════════════════════╝');
    console.log('\n');
