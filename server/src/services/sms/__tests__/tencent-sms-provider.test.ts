...
describe('TencentSmsProvider', () => {
  describe('配置验证', () => {
    ...
    it('应该检测到必要的环境变量', async () => {
      // 检查环境变量是否存在
      const secretId = process.env.TENCENT_SECRET_ID || process.env.TENCENT_SMS_SECRET_ID;
      const secretKey = process.env.TENCENT_SECRET_KEY || process.env.TENCENT_SMS_SECRET_KEY;
      ...
      console.log('- TENCENT_SMS_SIGN_NAME:', signName ? '✅ 已配置' : '❌ 未配置');
      console.log('- TENCENT_SMS_TEMPLATE_ID_VERIFICATION:', templateId ? '✅ 已配置' : '❌ 未配置');
