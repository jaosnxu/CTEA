...
export class SmsManager {
  ...
  private async checkRateLimit(
    ...
    try {
      ...
      `);
      const phoneMinuteRows = phoneMinuteResult[0] as unknown as Array<{
        count: number;
      }>;
      ...
      `);
      const phoneHourRows = phoneHourResult[0] as unknown as Array<{
        count: number;
      }>;
      ...
      `);
      const phoneDayRows = phoneDayResult[0] as unknown as Array<{
        count: number;
      }>;
      ...
      `);
      const ipMinuteRows = ipMinuteResult[0] as unknown as Array<{
        count: number;
      }>;
      ...
      `);
      const ipHourRows = ipHourResult[0] as unknown as Array<{ count: number }>;
