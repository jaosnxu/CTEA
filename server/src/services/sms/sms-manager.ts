...
export class SmsManager {
  ...
  private initProviders(): void {
    ...
    console.log(`[SmsManager] 已注册 Provider: ${Array.from(this.providers.keys()).join(', ')}`);
  }
  ...
  async sendSmsSecure(request: SecureSmsSendRequest): Promise<SmsSendResponse> {
    ...
    if (result.success) {
      ...
      console.log(`✅ 短信发送成功: provider=${result.provider}, messageId=${result.messageId}`);
    } else {
      await this.logSmsSend(phone, userIp, result.provider || 'UNKNOWN', false, result.errorCode);
   ...
   * 🔥 安全发送验证码（带验证码校验）
   */
  async sendVerificationCodeSecure(request: SecureVerificationCodeRequest): Promise<VerificationCodeResponse> {
    const { ticket, randstr, userIp, language = 'ru', phone } = request;
  ...
  private async sendSms(request: SmsSendRequest): Promise<SmsSendResponse> {
    ...
    if (providers.length === 0) {
      return {
        ...
        errorMessage: getLocalizedError('provider_unavailable', request.language),
      };
    }
   ...
   * 发送验证码（不带验证码校验，内部使用）
   */
  private async sendVerificationCode(request: VerificationCodeRequest): Promise<VerificationCodeResponse> {
    const providers = await this.getAvailableProviders();
   ...
   * 检查频率限制
   */
  private async checkRateLimit(phone: string, ip: string): Promise<{ allowed: boolean; reason?: string }> {
