...
const getConfig = () => ({
  secretId:
    process.env.TENCENT_SMS_SECRET_ID || process.env.TENCENT_SECRET_ID || "",
  secretKey:
    process.env.TENCENT_SMS_SECRET_KEY || process.env.TENCENT_SECRET_KEY || "",
  appId: process.env.TENCENT_SMS_APP_ID || "",
  signName: process.env.TENCENT_SMS_SIGN_NAME || "CHUTEA",
  templateIdVerification:
    process.env.TENCENT_SMS_TEMPLATE_ID_VERIFICATION || "",
  templateIdNotification:
    process.env.TENCENT_SMS_TEMPLATE_ID_NOTIFICATION || "",
...
export class TencentSmsProvider implements ISmsProvider {
  ...
  async sendSms(request: SmsSendRequest): Promise<SmsSendResponse> {
    ...
    console.log(
      `[TencentSmsProvider] 发送短信: phone=${phone.substring(0, 5)}***, type=${type}`
    );
    ...
    try {
      ...
      console.error(
        `[TencentSmsProvider] 发送失败: ${errorCode} - ${errorMessage}`
      );
  ...
  async sendVerificationCode(
    ...
    const {
      ...
      code: providedCode,
      language = "ru",
      expireSeconds = CODE_EXPIRY_SECONDS,
    } = request;
    ...
    console.log(
      `[TencentSmsProvider] 发送验证码: phone=${phone.substring(0, 5)}***, code=${code}`
    );
