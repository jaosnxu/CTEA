 ...
 * 构建翻译提示词
 */
function buildTranslationPrompt(text: string, targetLang: 'ru' | 'en', context?: string): string {
  ...
  const response = await fetch(DEEPSEEK_API_URL, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: '你是一个专业的多语言翻译助手，专注于茶饮品牌的本地化翻译。你的翻译准确、自然、符合目标语言的表达习惯。'
        },
        {
          role: 'user',
          content: prompt
 ...
 */
export async function translateText(textZh: string, context?: string): Promise<TranslationResult> {
  try {
    // 并行翻译俄语和英语
    ...
    const ruConfidence = Math.min(100, Math.max(60, 100 - Math.abs(ruRatio - 2) * 20));
    const enConfidence = Math.min(100, Math.max(60, 100 - Math.abs(enRatio - 3) * 15));
 ...
 * 仅翻译到俄语
 */
export async function translateToRussian(textZh: string, context?: string): Promise<{ success: boolean; text?: string; error?: string }> {
  try {
    const text = await callDeepSeekAPI(buildTranslationPrompt(textZh, 'ru', context));
 ...
 * 仅翻译到英语
 */
export async function translateToEnglish(textZh: string, context?: string): Promise<{ success: boolean; text?: string; error?: string }> {
  try {
    const text = await callDeepSeekAPI(buildTranslationPrompt(textZh, 'en', context));
    return { success: true, text };
  } catch (error) {
    return {
      success: false,
 ...
 */
export async function checkDeepSeekAvailability(): Promise<{ available: boolean; error?: string }> {
  if (!DEEPSEEK_API_KEY) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const errorText = await response.text();
      return { available: false, error: `API returned ${response.status}: ${errorText}` };
    }
  } catch (error) {
    return {
      available: false,
...
export function preprocessWithTerminology(
  textZh: string,
  targetLang: 'ru' | 'en'
): { processed: string; replacements: Array<{ original: string; replacement: string }> } {
