...
import { pgTable, serial, varchar, decimal, boolean, timestamp, text, index } from 'drizzle-orm/pg-core';

export const iikoShadowMenu = pgTable('iiko_shadow_menu', {
  id: serial('id').primaryKey(),
  
  // ===== iiko 商品信息 =====
  iikoProductId: varchar('iiko_product_id', { length: 255 }).notNull().unique(),
  iikoOrganizationId: varchar('iiko_organization_id', { length: 255 }).notNull(),
  ...
  iikoProductIdIdx: index('idx_iiko_shadow_menu_product_id').on(table.iikoProductId),
  
  // 索引：同步状态（用于筛选待审核商品）
  syncStatusIdx: index('idx_iiko_shadow_menu_sync_status').on(table.syncStatus),
  
  // 🔴 索引：价格警告（用于快速定位需要人工审核的商品）
  priceAlertIdx: index('idx_iiko_shadow_menu_price_alert').on(table.priceAlert),
}));
