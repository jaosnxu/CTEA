...
export function calculateBusinessDate(
  ...
  if (hour >= 0 && hour < 4) {
    ...
    const businessDate = orderDateTime
      .minus({ days: 1 })
      .toFormat("yyyy-MM-dd");
    console.log(
      `[BusinessDate] 订单时间: ${orderTime}, 小时: ${hour}, 营业日: ${businessDate} (归属前一天)`
    );
    return businessDate;
  } else {
    // 04:00 后：归属当天营业日
    ...
    console.log(
      `[BusinessDate] 订单时间: ${orderTime}, 小时: ${hour}, 营业日: ${businessDate} (归属当天)`
    );
    return businessDate;
  }
}
 ...
 * @returns 带有营业日的订单列表
 */
export function batchCalculateBusinessDate(
  orders: Array<{ orderTime: string; timezone: string }>
) {
