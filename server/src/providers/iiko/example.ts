...
async function main() {
  ...
  try {
    ...
    console.log(`   示例商品: ${menu[0].name} - ${menu[0].price} (成本: ${menu[0].cost})\n`);
    ...
    if (orderResponse.success) {
      ...
      const statusResponse = await iikoProvider.getOrderStatus(orderResponse.orderId);
      ...
      if (salesResponse.success) {
        console.log(`   ✓ 同步成功，获取到 ${salesResponse.salesData.length} 条销售记录`);
        if (salesResponse.salesData.length > 0) {
          const sale = salesResponse.salesData[0];
          console.log(`   示例销售记录: 订单 ${sale.orderId}, 金额 ${sale.totalAmount}\n`);
        }
      }
    } else {
    ...
    console.log('- 要切换到真实 iiko 环境，请在 .env 中设置 IIKO_PROVIDER=real');
