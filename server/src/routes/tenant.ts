...
import { organizations, stores, adminUsers, auditLogs } from '../../../drizzle/schema';
    ...
    if (!db) {
      ...
      const mockStores = [
        ...
        },
        {
          ...
          orgId: 3,
          code: 'SPB-002',
          name: { ru: 'Санкт-Петербург - Васильевский', zh: '圣彼得堡瓦西里岛店' },
...
router.get('/context', async (req: Request, res: Response) => {
  try {
    ...
    const orgId = req.headers['x-org-id'] ? parseInt(req.headers['x-org-id'] as string) : null;
    const storeId = req.headers['x-store-id'] ? parseInt(req.headers['x-store-id'] as string) : null;
    ...
    if (db) {
      ...
      stats = {
        ...
        stores: {
          ...
          inactive: Number(storeCount[0]?.count || 0) - Number(activeStoreCount[0]?.count || 0),
        },
      };
    }
