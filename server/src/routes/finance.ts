...
import { withdrawalRequests, influencers, auditLogs, users } from '../../../drizzle/schema';
...
router.get('/withdrawals', async (req: Request, res: Response) => {
  try {
    ...
    if (startDate) {
      conditions.push(gte(withdrawalRequests.createdAt, new Date(startDate as string)));
    }
    if (endDate) {
      conditions.push(lte(withdrawalRequests.createdAt, new Date(endDate as string)));
    }
    ...
    // 获取达人信息
    const influencerIds = Array.from(new Set(withdrawals.map((w: typeof withdrawals[0]) => w.influencerId)));
    ...
    if (influencerIds.length > 0) {
      const influencerList = await db
        .select()
        .from(influencers)
        .where(sql`${influencers.id} IN (${sql.join(influencerIds.map((id: number) => sql`${id}`), sql`, `)})`);
      
      // 获取用户信息
      const userIds = influencerList.map((i: typeof influencerList[0]) => i.userId).filter(Boolean) as number[];
      ...
      if (userIds.length > 0) {
        ...
        userMap = userList.reduce((acc: Record<number, any>, u: typeof userList[0]) => {
          acc[u.id] = u;
          return acc;
        }, {} as Record<number, any>);
      }
      
      influencerMap = influencerList.reduce((acc: Record<number, any>, inf: typeof influencerList[0]) => {
        acc[inf.id] = {
          ...inf,
    ...
    statsResult.forEach((s: { status: string | null; count: number; total: number }) => {
...
router.post('/withdrawals/:id/approve', async (req: Request, res: Response) => {
  try {
    ...
    const { id } = req.params;
    const { action, reason, adminId, adminName } = req.body as WithdrawalApprovalRequest;
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    const withdrawalStats = await db
      ...
      .from(withdrawalRequests)
      .groupBy(withdrawalRequests.status) as { status: string | null; count: number; total: number }[];
    ...
    // 待审批金额
    const pendingAmount = withdrawalStats.find((s: { status: string | null; count: number; total: number }) => s.status === 'PENDING')?.total || 0;
    const pendingCount = withdrawalStats.find((s: { status: string | null; count: number; total: number }) => s.status === 'PENDING')?.count || 0;
    
    // 已完成金额
    const completedAmount = withdrawalStats.find((s: { status: string | null; count: number; total: number }) => s.status === 'COMPLETED')?.total || 0;
    const completedCount = withdrawalStats.find((s: { status: string | null; count: number; total: number }) => s.status === 'COMPLETED')?.count || 0;
    ...
    res.json({
      ...
      data: {
        ...
        byStatus: withdrawalStats.map((s: { status: string | null; count: number; total: number }) => ({
          status: s.status,
