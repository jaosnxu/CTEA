...
import { requireAuth, requireAuthWithUser, optionalAuth, loadUser } from '../middleware/auth-middleware';
...
router.post('/login', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      // 根据错误码返回不同的 HTTP 状态码
      const statusCode = 
        result.errorCode === 'CODE_MISMATCH' || result.errorCode === 'CODE_EXPIRED' ? 401 :
        result.errorCode === 'MAX_ATTEMPTS_EXCEEDED' ? 429 :
        result.errorCode === 'CODE_NOT_FOUND' ? 404 :
        400;
    ...
    return res.status(500).json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: language === 'ru' ? 'Внутренняя ошибка сервера.' : '服务器内部错误',
      },
    });
  }
});
  ...
  try {
    ...
    if (result) {
      ...
      return res.status(401).json({
        ...
        error: {
          code: 'REFRESH_FAILED',
          message: language === 'ru' ? 'Не удалось обновить токен.' : 'Token 刷新失败',
        },
      });
    }
 ...
 * }
 */
router.get('/me', requireAuth, loadUser, async (req: Request, res: Response) => {
  const language = getRequestLanguage(req);
  ...
  try {
    if (!req.user) {
      return res.status(404).json({
        ...
        error: {
          code: 'USER_NOT_FOUND',
          message: language === 'ru' ? 'Пользователь не найден.' : '用户不存在',
        },
      });
    }
  ...
  try {
    ...
    if (success) {
      ...
      return res.status(500).json({
        ...
        error: {
          code: 'UPDATE_FAILED',
          message: language === 'ru' ? 'Не удалось обновить профиль.' : '更新失败',
        },
      });
    }
