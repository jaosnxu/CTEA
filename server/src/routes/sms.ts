...
import { getSmsVerificationService, VerificationPurpose } from '../services/sms-verification-service';
import { getSmsManager } from '../services/sms';
import { validatePhoneNumber, getLocalizedError } from '../services/sms/sms-provider.interface';
...
router.post('/send', async (req: Request, res: Response) => {
  ...
  try {
    const { phone, purpose = 'LOGIN', ticket, randstr } = req.body as SendCodeRequestBody;
    ...
    if (result.success) {
      ...
      // 根据错误码返回不同的 HTTP 状态码
      const statusCode = 
        result.errorCode === 'CAPTCHA_FAILED' || result.errorCode === 'CAPTCHA_REQUIRED' ? 403 :
...
router.post('/verify', async (req: Request, res: Response) => {
  ...
  try {
    const { phone, code, purpose = 'LOGIN' } = req.body as VerifyCodeRequestBody;
    ...
    if (!code) {
      return res.status(400).json({
        ...
        error: {
          code: 'MISSING_CODE',
          message: language === 'ru' ? 'Введите код подтверждения.' : '请输入验证码',
        },
      });
    }
    ...
    if (!/^\d{6}$/.test(code)) {
      return res.status(400).json({
        ...
        error: {
          ...
          message: language === 'ru' ? 'Код должен состоять из 6 цифр.' : '验证码必须是6位数字',
        },
      });
    }
    ...
    if (result.success) {
      ...
      // 根据错误码返回不同的 HTTP 状态码
      const statusCode = 
        result.errorCode === 'CODE_NOT_FOUND' || result.errorCode === 'CODE_EXPIRED' ? 404 :
        result.errorCode === 'CODE_MISMATCH' ? 401 :
