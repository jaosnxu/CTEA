...
router.put('/skus/:id', async (req: Request, res: Response) => {
  try {
    ...
    const { id } = req.params;
    const { isActive, isAvailableOnTV, isAvailableOnApp, isAvailableOnWeb, adminId, adminName } = req.body;
    ...
    await db.insert(auditLogs).values({
      ...
      action: 'UPDATE',
      diffBefore: {},
      diffAfter: { isActive, isAvailableOnTV, isAvailableOnApp, isAvailableOnWeb },
...
router.put('/stores/:id', async (req: Request, res: Response) => {
  try {
    ...
    const { id } = req.params;
    const { status, businessHours, tvEnabled, deliveryEnabled, pickupEnabled, adminId, adminName } = req.body;
    ...
    await db.insert(auditLogs).values({
      ...
      action: 'UPDATE',
      diffBefore: {},
      diffAfter: { status, businessHours, tvEnabled, deliveryEnabled, pickupEnabled },
      operatorId: adminId || 1,
      operatorType: 'ADMIN',
      operatorName: adminName || 'Admin',
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          total: alerts.length,
          critical: alerts.filter((a: typeof alerts[0]) => a.status === 'CRITICAL').length,
          low: alerts.filter((a: typeof alerts[0]) => a.status === 'LOW').length,
        },
      },
    });
  } catch (error: any) {
