    ...
    const modules: ModuleStatus[] = [
            ...
            ? [
                {
                  level: "warning",
                  message: {
                    ru: `${pendingWithdrawals} заявок на вывод ожидают обработки`,
                    zh: `${pendingWithdrawals} 笔提现申请待处理`,
                  },
      ...
      },
      {
        ...
        metrics: [
          {
            key: "total",
            label: { ru: "Всего SKU", zh: "总 SKU" },
            value: totalProducts,
          },
          {
            key: "active",
            label: { ru: "Активных", zh: "启用" },
            value: totalProducts,
          },
        ],
        alerts: [],
      ...
      },
      {
        ...
        metrics: [
          {
            key: "stores",
            label: { ru: "Магазины", zh: "门店" },
            value: totalStores,
          },
          {
            key: "orders",
            label: { ru: "Заказы", zh: "订单" },
            value: todayOrders,
          },
        ],
        alerts: [],
    ...
    const insights: AIInsight[] = [
      {
        ...
        description: {
          ru: "Продажи классического молочного чая выросли на 25% за последнюю неделю",
          zh: "经典奶茶销量过去一周增长 25%",
        },
      ...
      },
      {
        ...
        description: {
          ru: "Среднее время доставки можно сократить на 15% при оптимизации маршрутов",
          zh: "优化路线可将平均配送时间缩短 15%",
        },
        module: "operations",
        timestamp: new Date().toISOString(),
      },
      {
        ...
        description: {
          ru: "Запасы тапиоки в центральном магазине ниже минимума",
          zh: "中心店珍珠库存低于最低值",
        },
  ...
  try {
    ...
    const allInsights: AIInsight[] = [
      ...
      },
      {
        ...
        description: {
          ru: "3 новых блогера показали отличные результаты, рекомендуется повысить их уровень",
          zh: "3 位新达人表现优秀，建议提升等级",
        },
        module: "influencers",
        timestamp: new Date().toISOString(),
      },
      {
        ...
        description: {
          ru: "Исторические данные показывают, что акции на выходные увеличивают продажи на 30%",
          zh: "历史数据显示周末促销可提升销量 30%",
        },
        module: "marketing",
        timestamp: new Date().toISOString(),
      },
...
router.get("/financial-risk-report", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: {
        message: error.message || "Failed to get financial risk report",
      },
    });
  }
});
