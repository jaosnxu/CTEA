  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        console.log(`✅ ${endpoint.method} ${endpoint.path}: ${response.status} OK`);
        results.push({
          category: 'API',
          test: endpoint.name,
          status: 'PASS',
        ...
        console.log(`⚠️  ${endpoint.method} ${endpoint.path}: ${response.status} (endpoint may not exist yet)`);
        results.push({
          category: 'API',
          test: endpoint.name,
          status: 'WARN',
      ...
      if (error.code === 'ECONNREFUSED') {
        console.log(`⚠️  ${endpoint.method} ${endpoint.path}: Server not running`);
        results.push({
          category: 'API',
          test: endpoint.name,
          status: 'WARN',
  ...
  try {
    ...
    for (const check of checks) {
      if (check.count >= check.min) {
        ...
        console.log(`⚠️  ${check.name}: ${check.count} (min: ${check.min}) - Run setup to create test data`);
        results.push({
          category: 'Data Integrity',
          test: check.name,
          status: 'WARN',
    ...
    const ordersWithoutItems = ordersWithItems.filter(o => o.orderItems.length === 0);
...
function generateReport(): void {
  ...
  if (totalWarned > 0 || totalFailed > 0) {
    ...
    const apiIssues = results.filter(r => 
      r.category === 'API' && 
      r.message.includes('Server not running')
    );
