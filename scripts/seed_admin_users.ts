...
async function main() {
  ...
  const superAdmin = await prisma.adminUser.upsert({
    ...
    create: {
      ...
      name: { en: 'Super Administrator', ru: 'Супер Администратор', zh: '超级管理员' },
      role: 'HQ_ADMIN',
      status: 'ACTIVE',
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  console.log(`✅ Created super admin: ${superAdmin.username} (${superAdmin.id})`);
  ...
  const orgAdmin = await prisma.adminUser.upsert({
    ...
    create: {
      ...
      name: { en: 'Organization Administrator', ru: 'Администратор Организации', zh: '组织管理员' },
      role: 'ORG_ADMIN',
      status: 'ACTIVE',
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  const storeAdmin = await prisma.adminUser.upsert({
    ...
    create: {
      ...
      name: { en: 'Store Administrator', ru: 'Администратор Магазина', zh: '门店管理员' },
      role: 'STORE_MANAGER',
      status: 'ACTIVE',
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  console.log(`✅ Created store admin: ${storeAdmin.username} (${storeAdmin.id})`);
